...
## Standing notes
- Handoff email + admin dashboard added (decisions/006, 2026-07-28): closes the gap where the pilot partner had no non-technical way to read summaries (Render Shell requires technical skill they don't have). Email via Gmail SMTP (dedicated account only, never personal — App Passwords aren't send-only scoped) is optional/soft-configured; admin dashboard at `/admin.html` behind its own `ADMIN_CODE`. Owner still needs to: create a dedicated Gmail account + app password, and set `GMAIL_USER`/`GMAIL_APP_PASSWORD`/`OWNER_EMAIL`/`ADMIN_CODE` on Render, then redeploy. WhatsApp explicitly deferred until the project has paying traction (Business API verification is slow, disproportionate for POC).
