...
## Standing notes
- UI redesigned around real brand color (plum/purple from the actual logo), gradient header, mobile safe-area padding, plus clickable starter-question chips on the empty chat state (2026-07-28) — client-facing page only, admin dashboard stays plain. Live, owner-reviewed ("not bad"), not independently visually verified by the agent (no browser tool available this session).
- **Fixed a real production bug (2026-07-28):** `ERR_ERL_UNEXPECTED_X_FORWARDED_FOR` from Render's error logs — Express's default `trust proxy` setting didn't trust Render's reverse-proxy header, making `express-rate-limit`'s per-IP tracking unreliable (the actual mechanism the cost-control design depends on). Fixed with `app.set('trust proxy', 1)`, verified live via rate-limit headers afterward.
- Handoff email + admin dashboard added (decisions/006, 2026-07-28): closes the gap where the pilot partner had no non-technical way to read summaries (Render Shell requires technical skill they don't have). Email via Gmail SMTP (dedicated account `smbnotifications2@gmail.com`, never personal — App Passwords aren't send-only scoped) is optional/soft-configured; admin dashboard at `/admin.html` behind its own `ADMIN_CODE`. **Live and verified** — email, CC (`NOTIFY_CC_EMAIL`), and dashboard all confirmed working in production. WhatsApp explicitly deferred until the project has paying traction (Business API verification is slow, disproportionate for POC).
- **Caught and fixed:** `OWNER_EMAIL` was briefly set to a real client's address instead of the business owner's — would have leaked every future client's conversation data to that one client. No real harm done (caught before real client traffic started), but a reminder to double-check recipient env vars specifically, not just whether email sends successfully.
- **Prompt refinements (2026-07-28), live-verified:** content gaps now capture contact info directly instead of deflecting to "contact us yourself"; contact-info ask moved earlier (first couple of replies, honest timeout framing) instead of waiting for full event discovery; responses shortened significantly (explicit brevity instruction + `MAX_OUTPUT_TOKENS` 500→350) since the chat-style UI was reading as too long/bulleted, like an email rather than a conversation.
...
- Three reference images resolved (2026-07-28): all added to `.gitignore` (`src/docs/*.jpg`, `*.png`) - kept locally, never committed. Reviewing "Sample questions needed to understand client needs.jpg" (the business's own real discovery questionnaire) surfaced a genuine content gap: a whole second service line, "Drinks Service" (soft drinks, water, malt drinks, wine, beer, champagne - quoted by guest count), distinct from "Cocktail Service" (quoted by glass count) - the docs previously only covered Cocktail Service, which is exactly why the assistant had no answer on wine/beer questions earlier. Added a "Two Service Lines" section to `business-docs.txt` and rewrote the system prompt's discovery-question order to match the business's actual questionnaire (service line first, then the right follow-ups per line) instead of generic guessed questions.
